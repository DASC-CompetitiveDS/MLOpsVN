...
def put_folder(path, include_parent_path):
    client = Minio(
            "localhost:9009",
...
if __name__ == "__main__":
    ...
    parser.add_argument("--include_parent_path", type=lambda x: (str(x).lower() == "true"), default=False, 
                        help='nếu True, thêm đầy đủ đường dẫn; nếu False thì chỉ đẩy thư mục con')
    
    args = parser.parse_args()
    
    put_folder(args.path, args.include_parent_path)